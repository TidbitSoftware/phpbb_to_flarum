...
$result = $exportDbConnection->query("SELECT user_id, from_unixtime(user_regdate) as user_regdate, username_clean, user_email FROM forum_users");
...
$result = $exportDbConnection->query("SELECT forum_id, forum_name, forum_desc  FROM forum_forums");
...
$topicsQuery = $exportDbConnection->query("SELECT topic_id, topic_poster, forum_id, topic_title, topic_time FROM forum_topics ORDER BY topic_id DESC;");
...
if($topicCount)
{
	...
	while($topic = $topicsQuery->fetch_assoc())
	{
		...
		$participantsArr = [];
		$lastPosterID = 0;
		$sqlQuery = sprintf("SELECT * FROM forum_posts WHERE topic_id = %d;", $topic["topic_id"]);
		...
		if($postCount)
		{
			...
			while($post = $postsQuery->fetch_assoc())
			{
				...
				$postres= $importDbConnection->query($insertString . $formattedValuesStr);
                                if($postres === false) {
                                        echo "Wrong SQL: " . $query . " Error: " . $importDbConnection->error . " <br/>";
                                }
			}
		...
		$query = "INSERT INTO ". $importDBPrefix. "discussions (id, title, slug, created_at, comment_count, participant_count, first_post_id, last_post_id, user_id, last_posted_user_id, last_posted_at) VALUES( '$topicid', '$topicTitle', '$slug', '$discussionDate', '$postCount', '$count', (SELECT min(id) FROM flarum.posts where discussion_id=$topicid), (SELECT max(id) FROM flarum.posts where discussion_id=$topicid), '$poster', '$lastPosterID', '$discussionDate');";
		...
		$parentForum = $exportDbConnection->query("SELECT parent_id FROM forum_forums WHERE forum_id = " . $topic["forum_id"]);
...
$result = $exportDbConnection->query("SELECT user_id, topic_id FROM forum_topics_posted");
